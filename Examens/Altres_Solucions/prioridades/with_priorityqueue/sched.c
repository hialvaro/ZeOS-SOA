...
// Fork queue
struct list_head forkqueue;
// Priority queue AMR0811
struct list_head priorityqueue;
...
{
  // AMR0811
  if ((remaining_quantum==0)&&((!list_empty(&readyqueue))||(!list_empty(&priorityqueue)))) return 1;
...
void sched_next_rr(void)
{
  ...
  if (!list_empty(&readyqueue)) {
    ...
    if(nice){
        if(list_empty(&priorityqueue)) e = list_first(&readyqueue);
        else e = list_first(&priorityqueue);
    }
    else e = list_first(&readyqueue);
...
void schedule()
{
  ...
  if (needs_sched_rr())
  {
    if(current()->nice) update_process_state_rr(current(), &readyqueue);
    else update_process_state_rr(current(), &priorityqueue);
    sched_next_rr();
  }
}
